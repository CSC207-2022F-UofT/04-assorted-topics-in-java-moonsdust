...
class DrivableMap {
     ...
     */
    public boolean addDrivable(String keyy, Drivable obj) {
        if (!drivable_map.containsKey(keyy)) {
            drivable_map.put(keyy, obj);
            return true;
        }
        return false;
    }
    ...
    public List<Tradable> getTradable() {
        List<Tradable> allTradable = new ArrayList<Tradable>();
        for (Drivable obj : drivable_map.values()) {
            if (obj instanceof Tradable) {
                allTradable.add((Tradable) obj);
            }
        }
        return allTradable;
    }
